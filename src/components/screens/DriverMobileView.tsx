  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white p-5 rounded-2xl shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-indigo-400 font-bold text-xs uppercase tracking-wider">
            <Smartphone className="h-4 w-4" /> Driver Companion App
          </div>
          <KPIBadge type="Calculated" formula="Rule R6 Investigation Auto-Trigger" />
        </div>
        <div>
          <h1 className="text-xl font-black">Pre-Trip & Route Incident Logger</h1>
          <p className="text-xs text-slate-300">Mohamed Farsi — Driver ID: DRV-082</p>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div>
          <label className="text-xs font-bold text-slate-700 block mb-1">Select Assigned Coach / Van</label>
          <select
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-3 text-xs">
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              ...
              <div>
                ...
                <span className="font-bold text-slate-800">In {currentVehicle.scheduled_use_days} Days</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
          ...
          Report Driver Observed Issue / Noise
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            <label className="font-bold text-slate-700 block mb-1">Description of Symptoms</label>
            <textarea
              rows={4}
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Describe what you heard, saw, or felt during drive or pre-trip inspection..."
